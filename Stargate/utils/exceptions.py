...
def exception_handler(exc, context):
    ...
    if isinstance(exc, exceptions.APIException):
        ...
        response = APIResponse(code=2, message='failed', errors='服务器内部错误', status=status.HTTP_507_INSUFFICIENT_STORAGE)
    else:
        # 未知错误
        view = context['view']
        # 数据库记录异常
        ...
        write_error_logs(exc, context, detail)
        logger.error('[%s] %s' % (view, detail))
        response = APIResponse(code=2, message='failed', errors='服务器内部错误', status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    return response
