...
class HostView(CommonModelViewSet):
    ...
    def get_queryset(self):
        if self.request.query_params.get('env_id'):
            env_id = self.request.query_params['env_id']
            queryset = self.queryset.filter(environments_id=env_id).all()
            return queryset
        return super(HostView, self).get_queryset()
